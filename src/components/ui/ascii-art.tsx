...
export const AsciiArt: React.FC<AsciiArtProps> = ({
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      const containerWidth = containerRef.current?.clientWidth || cols;
      const containerHeight = containerRef.current?.clientHeight || cols;
      const containerAspect = containerHeight > 0 ? containerWidth / containerHeight : 1;
      const rows = Math.max(
        1,
        Math.floor((cols / containerAspect) * charAspectRatio)
      );

      canvas.width = cols;
      canvas.height = rows;

      const visualAspect = (cols / rows) * charAspectRatio;
  ...
  const drawCanvas = useCallback(
    (progress: number = 1, matrixProgress?: number) => {
      ...
      // Keep glyph proportions stable and apply object-fit style layout
      // at render time to avoid visual stretching in non-square containers.
      const baseWidth = cols * charAspectRatio;
      ...
      if (objectFit === "cover") {
        ...
        // fill: intentionally stretch to exact container size
        scale = 0;
      }

      const charWidth =
        objectFit === "fill" ? containerWidth / cols : charAspectRatio * scale;
      const charHeight =
        objectFit === "fill" ? containerHeight / rows : scale;
      const contentWidth = charWidth * cols;
      const contentHeight = charHeight * rows;
